resource "aws_lambda_function" "from_s3_existing_new_file" {
  ...
  s3_bucket         = data.aws_s3_bucket.s3_existing_mode_new_file[each.key].bucket
  s3_key            = lookup(local.s3_from_existing_new_file_cfg[each.key], "use_zip_file") ? aws_s3_object.upload_existing_zip[each.key].key : lookup(local.s3_from_existing_new_file_cfg[each.key], "generate_zip_from_file") ? aws_s3_object.upload_zip_file[each.key].key : lookup(local.s3_from_existing_new_file_cfg[each.key], "generate_zip_from_dir") ? aws_s3_object.upload_zip_dir[each.key].key : null
  s3_object_version = lookup(local.s3_from_existing_new_file_cfg[each.key], "ignore_version_changes_enabled") ? null : lookup(local.s3_from_existing_new_file_cfg[each.key], "use_zip_file") ? aws_s3_object.upload_existing_zip[each.key].version_id : lookup(local.s3_from_existing_new_file_cfg[each.key], "generate_zip_from_file") ? aws_s3_object.upload_zip_file[each.key].version_id : lookup(local.s3_from_existing_new_file_cfg[each.key], "generate_zip_from_dir") ? aws_s3_object.upload_zip_dir[each.key].version_id : null
